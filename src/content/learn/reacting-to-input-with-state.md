---
title: پاسخ به ورودی به کمک استیت
---

<Intro>
ری اکت یک روش اعلانی برای ایجاد تغییر در رابط کاربری فراهم کرده است.به جای دستکاری مستقیم
هر بخش رابط کاربری به تنهایی،شما استیت های متفاوتی را که کامپوننت مورد نظرتان می تواند داشته
...
* برشمردن استیت های بصری که کامپوننت شما می تواند داشته باشد
* چگونه تغییرات بین استیت های بصری مختلف را از طریق کد اجرا کنیم؟
...
function submitForm(answer) {
  // Pretend we're sending a request to the network.
...
1. استیت های بصری مختلف کامپوننت خود را **شناسایی کنید**
2. **تعیین کنید** چه چیزی باعث آن تغییرات استیت می شود
3. استیت را در حافظه با استفاده از `useState` **نشان دهید**
4. هرگونه متغیر استیت غیرضروری را **حذف کنید**
5. event handler ها را برای مقداردهی استیت **متصل کنید**

### قدم اول : استیت های بصری مختلف کامپوننت خود را شناسایی کنید {/*step-1-identify-your-components-different-visual-states*/}

در علوم کامپیوتر،ممکن است درباره ["استیت machine"](https://en.wikipedia.org/wiki/ماشین_حالات_متناهی) که در یکی از چندین “states” قرار دارد بشنوید.اگر با یک طراح کار می کنید،ممکن است نمونه هایی برای "استیت های بصری" مختلف دیده باشید.ری اکت در تقاطع طراحی و علوم کامپیوتر قرار دارد، بنابراین هر دوی این ایده ها منابع الهام هستند.

ابتدا،شما نیاز دارید که تمام "استیت" های مختلف رابط کاربری را که کاربر ممکن است ببیند را بصری سازی کنید :
...
درست مانند یک طراح، قبل از اینکه منطق را اضافه کنید می خواهید برای استیت های مختلف "نمونه های اولیه" یا "نمونه" ایجاد کنید.برای مثال، در اینجا یک نمونه فقط برای بخش بصری فرم آورده شده است.این نمونه با یک prop به نام `status` با مقدار پیشفرض `'empty'` کنترل می شود:
...
#### نمایش همزمان چندین استیت بصری {/*displaying-many-visual-states-at-once*/}

اگر یک کامپوننت دارای تعداد زیادی استیت های بصری باشد،نمایش همه آنها در یک صفحه راحت تر خواهد بود :
...
### قدم دوم : تعیین کنید چه چیزی باعث آن تغییرات استیت می شود {/*step-2-determine-what-triggers-those-state-changes*/}

شما می توانید در پاسخ به دو نوع ورودی بروزرسانی های استیت را فعال کنید:
...
در هر دو مورد، **شما باید برای بروزرسانی رابط کاربری [متغیرهای استیت](/learn/state-a-components-memory#anatomy-of-usestate) را تنظیم کنید.** برای فرمی که در حال توسعه آن هستید، نیاز به تغییر استیت در پاسخ به چند ورودی متفاوت دارید: 

* **تغییر ورودی متن** (کاربر) باید آن را از استیت *خالی* به *درحال تایپ* یا برعکس تغییر دهد، بسته به اینکه ورودی متن خالی است یا خیر.
* **کلیک بر روی دکمه ارسال** (کاربر) باید آن را به استیت *در حال ارسال* تغییر دهد.
* **پاسخ موفق شبکه** (کامپیوتر) باید آن را به استیت *موفقیت* تغییر دهد.
* **پاسخ ناموفق شبکه** (کامپیوتر) باید آن را به استیت *خطا* با پیغام خطای متناسب تغییر دهد.
...
برای کمک به تصویر سازی این گردش کار، سعی کنید هر استیت را به عنوان یک دایره برچسب دار، و هر تغییر بین دو استیت را به عنوان یک پیکان ترسیم کنید.به این ترتیب می توانید چندین گردش کار را ترسیم کرده و باگ ها را قبل از پیاده سازی حل کنید.
...
### قدم سوم:  استیت را در حافظه با استفاده از `useState` نشان دهید {/*step-3-represent-the-state-in-memory-with-usestate*/}

حال شما باید استیت های بصری کامپوننت خود را در حافظه با [`useState`](/reference/react/useState) نشان دهید. سادگی کلید است: هر قطعه استیت یک "قطعه متحرک" است، و **شما کمترین تعداد "قطعه متحرک" ممکن را می خواهید.** پیچیدگی بیشتر به باگهای بیشتر منجر می شود!

با استیت ای شروع کنید که *حتما باید* وجود داشته باشد. مثلا، شما به `answer` برای ذخیره ورودی، و به `error`(اگر موجود باشد) برای ذخیره آخرین error نیاز دارید:
...
سپس، شما نیاز به یک متغیر استیت دارید که نمایانگر استیت های بصری که می خواهید نمایش داده شوند باشد. معمولا بیش از یک راه برای نمایش آن در حافظه وجود دارد،پس شما باید با آن آزمایش کنید.

اگر شما در تلاش هستید که به سرعت به بهترین راه دست یابید با افزودن تعداد کافی استیت آغاز کنید که *قطعا* مطمئن باشید تمام حالات بصری ممکن پوشش داده شده است:
...
احتمالا اولین ایده شما بهترین نخواهد بود، اما اشکالی ندارد--بازسازی استیت بخشی از فرایند است!

### قدم چهارم: هرگونه متغیر استیت  غیرضروری را حذف کنید {/*step-4-remove-any-non-essential-state-variables*/}

شما می خواهید از تکرار محتوای استیت جلوگیری کنید بنابراین فقط چیزی که ضروری است را دنبال می کنید. صرف زمان اندک روی بازسازی ساختار استیت باعث فهم ساده تر کامپوننت شما، کاهش تکرار، و اجتناب از معانی ناخواسته خواهد شد. هدف شما **جلوگیری از مواردی است که استیت موجود در حافظه، رابط کاربری صحیحی که شما بخواهید کاربر ببیند را نمایش نمی دهد.** (مثلا، شما هرگز نمی خواهید که همزمان پیغام خطا نمایش داده شود و فیلد ورودی هم غیرفعال باشد، یا کاربر قادر به تصحیح خطا نباشد!)

اینجا تعدادی پرسش وجود دارد که می توانید درباره متغیرهای استیت بپرسید:

* **آیا این استیت باعث تناقض می شود؟** مثلا، `isTyping` و `isSubmitting` نمیتوانند همزمان `true` باشند. یک تناقض معمولا به این معناست که یک استیت به اندازه کافی محدود نیست. چهار ترکیب ممکن از دو مقدار بولین وجود دارد، اما تنها سه مورد با استیت های معتبر مطابقت دارند. برای حذف استیت "غیرممکن"، می توانید این موارد را در یک `status` ترکیب کنید که باید یکی از این سه مقادیر باشد: `'typing'`، `'submitting'`، یا `'success'`.
* **آیا همان اطلاعات از قبل در استیت دیگری موجود است؟** یک تناقض دیگر: `isEmpty` و `isTyping` نمی توانند همزمان `true` باشند. با جدا کردن آن ها به عنوان متغیرهای استیت، خطر عدم هماهنگی بین آنها و تولید باگ وجود دارد. خوشبختانه، شما می توانید `isEmpty` را حذف کنید و به جای آن `answer.length === 0` را بررسی کنید.
* **آیا می توانید همان اطلاعات را از معکوس یک متغیر استیت دیگر به دست آورید؟** نیازی به `isError` نیست زیرا شما می توانید `error !== null` را به جای آن بررسی کنید.

با جمع بندی این بخش، شما به سه متغیر استیت *ضروری* محدود شده اید (کاهش یافته از 7!):
...
#### حذف استیت های "غیرممکن" با استفاده از یک ردیوسر {/*eliminating-impossible-states-with-a-reducer*/}

این سه متغیر نمایانگری مناسب از استیت این فرم هستند. اگرچه هنوز برخی از استیت های میانی وجود دارند که کاملا منطقی نیستند. مثلا، وقتی که `status` `'success'` است یک `error` غیر null منطقی نیست. برای مدلسازی دقیقتر استیت، شما می توانید [آن را به یک ردیوسر منتقل کنید.](/learn/extracting-state-logic-into-a-reducer) ردیوسر ها به شما این امکان را می دهند که چندین متغیر استیت را در یک شی واحد ترکیب کنید و تمامی منطق مربوطه را یکپارچه سازید!

</DeepDive>

### قدم پنجم: event handler ها را برای مقداردهی استیت متصل کنید {/*step-5-connect-the-event-handlers-to-set-state*/}

در نهایت، event handler هایی را ایجاد کنید که استیت را بروزرسانی کند. در زیر فرم نهایی، با تمامی event handler های متصل موجود است:
...
با وجود اینکه این کد طولانی تر از مثال دستوری اصلی است، اما به مراتب حساسیت کمتری دارد. تشریح همه تعاملها به عنوان تغییرات استیت بعدا به شما اجازه معرفی استیت های جدید بصری را بدون تاثیر روی موارد موجود می دهد. همچنین به شما اجازه می دهد هرآنچه که باید در هر استیت نمایش داده شود را بدون تغییر منطق تعامل تغییر دهید.

<Recap>

* برنامه نویسی اعلانی به معنای توصیف رابط کاربری برای هر استیت بصری به جای مدیریت جزئیات رابط کاربری است (دستوری).
* هنگام توسعه یک کامپوننت:
  1. تمامی استیت های بصری آن را شناسایی کنید.
  2. تغییرات استیت توسط عاملهای انسانی و کامپیوتری را تعیین کنید.
  3. استیت را با استفاده از `useState` مدل کنید.
  4. استیت های غیرضروری را به منظور اجتناب از باگها و تناقضات حذف کنید.
  5. event handler ها را برای تنظیم استیت متصل کنید.
...
این کامپوننت دو نوع استیت بصری دارد: زمانی که تصویر فعال است، و زمانی که تصویر غیرفعال است:
...
یک متغیر استیت بولین به تنهایی کافی است تا فعال یا غیرفعال بودن تصویر را نگهداری کند. وظیفه اصلی حذف یا اضافه کردن کلاسهای CSS بود. اگرچه، در ری اکت شما نیاز دارید به جای آنکه المنت های رابط کاربری را *دستکاری کنید* آنچه که می خواهید ببینید را *توصیف کنید*. بنابراین لازم است که هر دو کلاس CSS را براساس استیت فعلی محاسبه کنید. همچنین شما باید [انتشار را متوقف کنید](/learn/responding-to-events#stopping-propagation) تا کلیک روی تصویر به عنوان کلیک بر روی پس زمینه ثبت نگردد.
...
بخاطر داشته باشید که اگر دو قطعه مختلف JSX یک درخت مشابه را توصیف کنند، تودرتویی آنها (اولین `<div>` → اولین `<img>`) باید منظم باشد.درغیراینصورت، تغییر وضعیت `isActive` باعث بازسازی کل زیردرخت شده و [استیت آن را بازنشانی می کند.](/learn/preserving-and-resetting-state) به این دلیل است که اگر یک درخت JSX مشابه در هردو حالت بازگردانده شود، بهتر است آنها را به عنوان یک قطعه JSX واحد بنویسید.
...
شما به دو متغیر استیت برای نگهداری مقادیر ورودی نیاز خواهی داشت: `firstName` و `lastName`. همچنین شما به یک متغیر استیت `isEditing` نیاز دارید که نگهدارنده وضعیت نمایش یا عدم نمایش ورودی ها باشد. شما _نباید_ به یک متغیر `fullName` نیاز داشته باشید چرا که نام کامل همیشه از `firstName` و `lastName` به دست می آید.
...
تصور کنید ری اکت وجود نداشت. آیا می توانید این کد را به گونه ای بازسازی کنید که منطق با حساسیت کمتر و بیشتر شبیه نسخه ری اکت شود؟اگر مانند ری اکت استیت به صورت صریح باشد، به چه شکل خواهد بود؟
...
تابع `updateDOM` که نوشتید نشان می دهد وقتی که شما استیت را ست می کنید ری اکت در پس زمینه چه کاری انجام می دهد. (اگرچه، ری اکت از تغییر prop های DOM که پس از آخرین مرتبه ست شدن تغییری نداشته اند اجتناب می کند.) 
