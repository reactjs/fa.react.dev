...
هنگام استفاده از استایل‌های درخط در ری‌اکت، می‌توانید از `React.CSSProperties` برای توصیف شیء ارسال‌شده به پراپ `style` استفاده کنید. این type اتحادی از تمام ویژگی‌های ممکن CSS است و روشی خوب برای اطمینان از ارسال ویژگی‌های معتبر CSS به پراپ `style` و دریافت تکمیل خودکار در ویرایشگر شما است.
