...
`prerender` یک درخت ری‌اکت را به یک رشتهٔ HTML استاتیک با استفاده از [Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) رندر می‌کند.
...
این API به [Web Streams](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) وابسته است. برای Node.js، به‌جای آن از [`prerenderToNodeStream`](/reference/react-dom/static/prerenderToNodeStream) استفاده کنید.
...
برای رندر اپلیکیشن خود به HTML استاتیک، `prerender` را فراخوانی کنید.
...
* **اختیاری** `options`: یک شیء با گزینه‌های تولید استاتیک.
  ...
  * **اختیاری** `onError`: یک کالبک که هر زمان که یک خطای سرور رخ می‌دهد فعال می‌شود، چه [قابل بازیابی](/reference/react-dom/server/renderToReadableStream#recovering-from-errors-outside-the-shell) چه [غیرقابل بازیابی.](/reference/react-dom/server/renderToReadableStream#recovering-from-errors-inside-the-shell) به‌طور پیش‌فرض، فقط `console.error` را فراخوانی می‌کند. اگر آن را برای [ثبت گزارش‌های خرابی](/reference/react-dom/server/renderToReadableStream#logging-crashes-on-the-server) بازنویسی می‌کنید، مطمئن شوید همچنان `console.error` را فراخوانی می‌کنید. همچنین می‌توانید از آن برای [تنظیم کد وضعیت](/reference/react-dom/server/renderToReadableStream#setting-the-status-code) قبل از emit شدن shell استفاده کنید.
...
`prerender` یک Promise برمی‌گرداند:
- اگر رندر موفق باشد، Promise با شیء‌ای resolve می‌شود که شامل:
  - `prelude`: یک [Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) از HTML. می‌توانید از این stream برای ارسال پاسخ به‌صورت chunks استفاده کنید، یا می‌توانید کل stream را در یک رشته بخوانید.
- اگر رندر ناموفق باشد، Promise رد (reject) می‌شود. [از این برای خروجی دادن یک shell جایگزین استفاده کنید.](/reference/react-dom/server/renderToReadableStream#recovering-from-errors-inside-the-shell)
...
برای رندر درخت ری‌اکت خود به HTML استاتیک در یک [Readable Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream)، `prerender` را فراخوانی کنید:
...
برای رندر اپلیکیشن خود به یک رشتهٔ HTML استاتیک، `prerender` را فراخوانی کنید:
...
`prerender` قبل از اتمام تولید HTML استاتیک و resolve شدن منتظر بارگذاری تمام داده‌ها می‌ماند. مثلاً، یک صفحهٔ پروفایل را در نظر بگیرید که یک cover، یک نوار کناری با دوستان و عکس‌ها، و فهرستی از پست‌ها را نشان می‌دهد:
...
پاسخ `prerender` منتظر می‌ماند تا کل اپلیکیشن رندر را تمام کند، از جمله انتظار برای resolve شدن تمام مرزهای ساسپنس، قبل از resolve شدن. این برای تولید سایت استاتیک (SSG) به‌صورت از پیش طراحی شده و از استریم کردن محتوای بیشتر هنگام بارگذاری پشتیبانی نمی‌کند.

برای استریم کردن محتوا هنگام بارگذاری، از یک API رندر استریمی سمت سرور مانند [renderToReadableStream](/reference/react-dom/server/renderToReadableStream) استفاده کنید.
