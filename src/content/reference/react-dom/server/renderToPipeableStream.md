...
`renderToPipeableStream` یک درخت ری‌اکت را به یک [Node.js Stream](https://nodejs.org/api/stream.html) قابل pipe رندر می‌کند.
...
این API مخصوص Node.js است. محیط‌هایی با [Web Streams](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) مانند Deno و runtimeهای edge مدرن باید به‌جای آن از [`renderToReadableStream`](/reference/react-dom/server/renderToReadableStream) استفاده کنند.
...
برای رندر درخت ری‌اکت خود به‌صورت HTML در یک [Node.js Stream](https://nodejs.org/api/stream.html#writable-streams)، `renderToPipeableStream` را فراخوانی کنید.
...
* **اختیاری** `options`: یک شیء با گزینه‌های streaming.
  ...
  * **اختیاری** `onAllReady`: یک کالبک که وقتی تمام رندر کامل شود فعال می‌شود، شامل هر دو [shell](#specifying-what-goes-into-the-shell) و تمام [محتوای](#streaming-more-content-as-it-loads) اضافی. می‌توانید از این به‌جای `onShellReady` [برای خزشگرها و تولید استاتیک](#waiting-for-all-content-to-load-for-crawlers-and-static-generation) استفاده کنید. اگر streaming را از اینجا آغاز کنید، هیچ بارگذاری پیشرونده‌ای دریافت نخواهید کرد. stream حاوی HTML نهایی خواهد بود.
  ...
  * **اختیاری** `onShellReady`: یک کالبک که بلافاصله پس از رندر [shell اولیه](#specifying-what-goes-into-the-shell) فعال می‌شود. می‌توانید [کد وضعیت را تنظیم کنید](#setting-the-status-code) و `pipe` را برای شروع streaming اینجا فراخوانی کنید. ری‌اکت [محتوای اضافی](#streaming-more-content-as-it-loads) را پس از shell همراه با تگ‌های `<script>` درون‌خطی که fallbackهای بارگذاری HTML را با محتوا جایگزین می‌کنند، stream می‌کند.
  * **اختیاری** `onShellError`: یک کالبک که اگر خطایی در رندر shell اولیه وجود داشته باشد فعال می‌شود. خطا را به‌عنوان آرگومان دریافت می‌کند. هنوز هیچ بایتی از stream emit نشده، و نه `onShellReady` و نه `onAllReady` فراخوانی نخواهند شد، بنابراین می‌توانید [یک shell HTML جایگزین خروجی دهید.](#recovering-from-errors-inside-the-shell)
...
`renderToPipeableStream` یک شیء با دو متد برمی‌گرداند:

* `pipe` HTML را در [Writable Node.js Stream](https://nodejs.org/api/stream.html#writable-streams) ارائه‌شده خروجی می‌دهد. اگر می‌خواهید streaming را فعال کنید `pipe` را در `onShellReady` فراخوانی کنید، یا در `onAllReady` برای خزشگرها و تولید استاتیک.
* `abort` به شما اجازه می‌دهد [رندر سمت سرور را لغو کنید](#aborting-server-rendering) و بقیه را در کلاینت رندر کنید.

---

## استفاده {/*usage*/}

### رندر یک درخت ری‌اکت به‌صورت HTML در یک Node.js Stream {/*rendering-a-react-tree-as-html-to-a-nodejs-stream*/}
...
کالبک `onShellReady` وقتی کل shell رندر شده باشد فعال می‌شود. معمولاً، streaming را از آنجا آغاز می‌کنید:
...
تا زمانی که `onShellReady` فعال می‌شود، کامپوننت‌ها در مرزهای `<Suspense>` تودرتو ممکن است همچنان در حال بارگذاری داده‌ها باشند.
...
اگر خطایی هنگام رندر این کامپوننت‌ها رخ دهد، ری‌اکت HTML معناداری برای ارسال به کلاینت نخواهد داشت. `onShellError` را بازنویسی کنید تا به‌عنوان آخرین راه‌حل، HTML جایگزینی که به رندر سمت سرور وابسته نیست ارسال کنید:
...
اگر هنگام تولید shell خطایی وجود داشته باشد، هم `onError` و هم `onShellError` فعال می‌شوند. از `onError` برای گزارش خطا و از `onShellError` برای ارسال سند HTML جایگزین استفاده کنید. HTML جایگزین شما لزوماً یک صفحهٔ خطا نیست. به‌جای آن، می‌توانید یک shell جایگزین include کنید که اپلیکیشن شما را فقط روی کلاینت رندر می‌کند.
...
با [تقسیم اپلیکیشن خود](#specifying-what-goes-into-the-shell) به shell (بالای تمام مرزهای `<Suspense>`) و بقیهٔ محتوا، قبلاً بخشی از این مشکل را حل کرده‌اید. اگر shell خطا کند، کالبک `onShellError` را دریافت می‌کنید که به شما اجازه می‌دهد کد وضعیت خطا را تنظیم کنید. در غیر این صورت، می‌دانید که اپلیکیشن ممکن است روی کلاینت بازیابی شود، بنابراین می‌توانید «OK» ارسال کنید.
...
اگر کامپوننتی *بیرون* shell (یعنی داخل یک مرز `<Suspense>`) خطا پرتاب کند، ری‌اکت رندر را متوقف نخواهد کرد. این بدان معناست که کالبک `onError` فعال خواهد شد، اما همچنان به‌جای `onShellError` کالبک `onShellReady` را دریافت خواهید کرد. این به این دلیل است که ری‌اکت سعی می‌کند از آن خطا روی کلاینت بازیابی کند، [همانطور که در بالا توضیح داده شد.](#recovering-from-errors-outside-the-shell)
...
می‌توانید با کالبک `onAllReady` برای بارگذاری تمام محتوا انتظار کنید:
