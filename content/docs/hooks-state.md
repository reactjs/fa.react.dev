...
در [صفحه‌ی قبل](/docs/hooks-intro.html) هوک‌ها را با این مثال معرفی کردیم:
...
ما با استفاده از مقایسه کردن این کد با یک کلاس مشابه شروع به یادگیری هوک‌ها می‌کنیم.
...
در این کلاس, ما مقدار اولیه‌ی state `count` را با مساوی قرار دادن مقدار `this.state` با `{ count: 0 }` در constructor برابر `0` قرار داده‌ایم:
...
ما یک متغیر state به نام `count` تعریف کرده‌ایم, و مقدار آن را برابر `0` قرار داده‌ایم. ری‌اکت مقدار این state را میان رندر کردن دوباره‌ی کامپوننت حفظ می‌کند, و جدیدترین مقدار آن را برای تابع فراهم می‌کند. اگر ما بخواهم مقدار `count` را بروزرسانی کنیم, می‌توانیم از `setCount` استفاده کنیم.
...
وقتی که ما بخواهیم count را در یک کلاس نمایش دهیم, مقدار آن را از `this.state.count` می‌خوانیم:
...
## بروزرسانی State {#updating-state}

در کلاس, ما باید با استفاده از `this.setState()` مقدار state `count` را بروزرسانی کنیم:
...
حالا بیاید **خلاصه چیزهایی را که یادگرفته‌ایم را خط به خط برسی کنیم** و بفهمیم چه چیزهایی را یاد گرفته‌ایم.
...
* **خط 1:** ما هوک `useState` را از ری‌اکت import کردیم. که به شما اجازه‌ی استفاده از state را در تابع می‌دهد.
* **خط 4:** در تابع کامپوننت `Example`, ما با استفاده از هوک `useState` یک متغیر state تعریف کرده‌ایم. که بر اساس اسم‌هایی که به آن می‌دهیم یک جفت مقدار را بر می‌گرداند. ما اسم متغیر خود را را `count` گذاشتیم چون مقدار تعداد کلیک‌هایی که روی دکمه می‌کنیم را در خود نگه می‌دارد. ما مقدار اولیه آن را بوسیله ی پاس دادن  `0` به عنوان آرگومان به `useState` برابر `0` قرار داده‌ایم. مقدار دومی که `useState` برمی‌گرداند یک تابع است. این تابع به شما اجازه می‌دهد که مقدار `count` را بروزرسانی کنید پس ما اسم آن را `setCount` می‌گذاریم.
* **خط 9:** وقتی که کاربر روی دکمه کلیک می‌کند, ما با استفاده از `setCount` مقدار جدید را به state `count` می‌دهیم. سپس ری‌اکت کامپوننت `Example` را دوباره رندر می‌کند, و مقدار جدید `count` را به آن پاس می‌دهد.

حجم مطالب ممکن است باری یادگیری در اولین بار زیاد به نظر برسد. ولی عجله نکنید! اگر شما هر کدام از توضیحات را متوجه نشده‌اید, مثال بالا را دوباره بخوانید و سعی کنید یک بار دیگر مطالب را از بالا به پایین با دقت بخوانید. ما به شما این قول را می‌دهیم اگر یکبار دیگر تلاش به خواندن این مطلب بکنید "فراموش می‌کنید" که که state در کلاس‌ها چگونه کار می‌کند.

### نکته: Square Brackets([]) چه کاری انجام می‌دهد؟ {#tip-what-do-square-brackets-mean}

شاید متوجه استفاده از square brackets([]) موقع تعریف متغیر state شده‌باشید:
...
این یک syntax جاوا‌اسکریپت به نام ["array destructuring"](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment#Array_destructuring) است. و به این معنی است که که ما دو متغیر به نام‌های  `fruit` و `setFruit` تعریف کرده‌ایم, که `fruit` برابر با اولین مقدار و `setFruit` برابر با دومین مقداری می‌شود که توسط `useState` بازگردانده می‌شوند. کد زیر مشابه همین کار را انجام می‌دهد:

```js
  var fruitStateVariable = useState('banana'); // یه جفت متغیر را باز می‌گرداند
  var fruit = fruitStateVariable[0]; // مقدار اول
  var setFruit = fruitStateVariable[1]; // مقدار دوم
```

وقتی که ما توسط `useState` یک متغیر state تعریف می‌کنیم, `useState` یک جفت متغیر را بازمی‌گرداند — یک آرایه با دو آیتم. آیتم اول حاوی مقدار متغیر است, و آیتم دوم یک تابع برای بروزرسانی مقدار آن است. استفاده از `[0]` و `[1]` برای دسترسی ممکن است مقداری گمراه کننده باشد چون ما موقع کار با آرایه‌ از آن‌ها استفاده می‌کنیم  برای همین ما به جای آن از  array destructuring استفاده می‌کنیم.

>نکته
>
>شاید کنجکاو باشید که بدانید که ری‌اکت چگونه میفهمد که هر `useState` با کدام کامپوننت مطابقت دارد در صورتی که ما هیچ چیزی مانند کلید واژه‌ی `this` را به ری‌اکت پاس نمی‌دهیم. ما به [این سوال](/docs/hooks-faq.html#how-does-react-associate-hook-calls-with-components) و سوالات دیگر دی بخش پرسش و پاسخ جواب داده‌ایم.
...
در کامپوننت بالا, ما متغیر‌های `age`, `fruit`, و `todos` را داریم, و می‌توانیم هر کدام را جداگانه بروزرسانی کنیم:
...
شما **مجبور نیستید** متغیرهای state زیادی تعریف کنید. چون متغیرهای state توانایی نگه‌داری object و array را هم دارد, پس می‌توانید مقدار های مربوط به هم را به عنوان یک گروه تعریف کنید. با این حال, برخلاف `this.setState` در کلاس, بروزرسانی یک متغیر state همیشه به جای ترکیب کردن از *جایگزین* کردن استفاده می‌کند.
