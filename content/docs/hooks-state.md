...
شما **مجبور نیستید** متغیرهای state زیادی تعریف کنید. چون متغیرهای state توانایی نگه‌داری object و array را هم دارد، پس می‌توانید مقدارهای مربوط به هم را به عنوان یک گروه تعریف کنید. با این حال، برخلاف `this.setState` در کلاس، بروزرسانی یک متغیر state همیشه به جای ترکیب کردن از *جایگزین* کردن استفاده می‌کند.
