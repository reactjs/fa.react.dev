...
Context راهی را برای انتقال داده در درخت کامپوننت بدون نیاز به انتقال دستیه props به پایین در هر سطح فراهم می‌کند.
...
**اگر فقط می‌خواهید که از انتقال برخی از داده‌ها بین بسیاری از سطح‌ها اجتناب کنید, اغلب [ترکیب‌بندی کامپوننت](/docs/composition-vs-inheritance.html) راه‌حلی ساده‌تر از context می‌باشد.**
...
این الگو در بسیاری از مواردی که باید یک فرزند را از والدین نزدیک آن جدا کرد کافی است. در صورتی که فرزند قبل از رندر شدن نیاز به برقراری ارتباط با والدین را داشته باشد٬ شما می‌توانید این قضیه را با [رندر کردن propها](/docs/render-props.html) ادامه دهید.
...
کد بالا یک شیٔ context ایجاد می‌کند. وقتی ری‌اکت یک کامپوننتی را رندر می‌کند که به این شیء context ارجاع می‌کند مقدار context فعلی را از نزدیک‌ترین `Provider` مطابق بالاتر از خود در درخت می‌خواند.
