...
المنت‌های فرم HTML در نحوه‌ی کارکرد با المنت‌های دیگر DOM در ری‌اکت تفاوت جزیی دارند، به این دلیل که در حالت طبیعی این المنت‌ها وضعیت [ورودی] را در خود ‌نگه‌داری ‌می‌کنند. برای مثال، این کد فرم که ‌به‌صورت HTML ساده نوشته شده است از کاربر یک نام دریافت ‌می‌کند:
...
در کل، این نوع ساختار باعث ‌می‌شود که تمام المنت‌های `<input type="text">`, `<textarea>` و `<select>`، عملکرد بسیار مشابهی داشته باشند. به این صورت که همگی یک خصوصیت `value` دریافت ‌می‌کنند که ‌می‌توان با آن یک کامپوننت کنترل‌شده را پیاده سازی کنید.
