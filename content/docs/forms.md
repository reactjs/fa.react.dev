...
تعریف value روی prop یک [کامپوننت کنترل‌شده](/docs/forms.html#controlled-components) باعث جلوگیری کاربر از تغییر ورودی ‌می‌شود مگر این که خودتان بخواهید. اگر خصوصیت `value` را تعریف کرده‌اید اما ورودی هنوز قابل‌ ویرایش است، ممکن است مقدار `value` را ‌به‌صورت تصادفی به `undefined` و یا `null` قرار داده باشید.
