...
اگر فکر می‌کنید که در جایی باید رویداد ذاتی خود مرورگر را استفاده کنید، کافی است که `nativeEvent` را به عنوان یک attribute اضافه کنید تا به آن دسترسی یابید. رویدادهای سینتاتیک متفاوت از رویداد‌های ذاتی مرورگر هستند و نباید هم ارز آن‌ها درنظر گرفته‌شوند. برای مثال، در `onMouseLeave` مقدار `event.nativeEvent` به رویداد `mouseout` اشاره دارد. این مورد مشخص بخشی از API عمومی نیست و ممکن است در آینده تغییر کند. هر آبجکت `SyntheticEvent`، attribute های زیر دارد:
